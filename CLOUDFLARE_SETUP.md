...
After connecting the correct Cloudflare account, `npm run deploy` publishes the build. Supabase values are compiled into the browser bundle, so changing only Worker runtime variables is insufficient; rebuild when the public project configuration changes.
...
A Cloudflare account/deployment connection has not been established in this session, so no public site URL is claimed.
